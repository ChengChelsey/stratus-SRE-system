...
PROJECT="${1:-$HOME/xiaocheng/projects/aiops-stratus/stratus}"
cd "$PROJECT"

if [[ ! -f test_agent.sh || ! -d eval || ! -d src/stratus ]]; then
  echo "ERROR: not a STRATUS project root: $PROJECT" >&2
  exit 2
fi

echo "Project root: $PWD"

mkdir -p evolution planner planner/data evolution/trajectories evolution/legacy
touch evolution/__init__.py planner/__init__.py

stamp="$(date +%F_%H%M%S)"

# Keep old misplaced schema / outputs instead of deleting them.
if [[ -f evolution/schema.py ]]; then
  mv evolution/schema.py "evolution/legacy/schema.py.legacy.$stamp"
fi
if [[ -f evolution/trajectory.jsonl ]]; then
  mv evolution/trajectory.jsonl "evolution/legacy/trajectory.jsonl.legacy.$stamp"
fi

cp planner/schema.py "planner/schema.py.bak.$stamp" 2>/dev/null || true
cp evolution/extract_trajectory.py "evolution/extract_trajectory.py.bak.$stamp" 2>/dev/null || true
cp evolution/build_tool_data.py "evolution/build_tool_data.py.bak.$stamp" 2>/dev/null || true

cat > planner/schema.py <<'PY_SCHEMA'
...
cat > evolution/extract_trajectory.py <<'PY_EXTRACT'
...
cat > evolution/build_tool_data.py <<'PY_BUILD'
...
python -m py_compile planner/schema.py evolution/extract_trajectory.py evolution/build_tool_data.py

echo
echo "Wrote:"
ls -lh planner/schema.py evolution/extract_trajectory.py evolution/build_tool_data.py

echo
echo "Current layout:"
find evolution planner -maxdepth 2 -type f | sort
