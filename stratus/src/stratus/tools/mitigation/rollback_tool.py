...
from stratus.tools.kubectl.kubectl import KubeCtl
from stratus.utils import _cleanup_kubernetes_yaml
...
class RollbackToolSchema(BaseModel):
    """Schema for RollbackTool input."""

    argument: str = Field(
        title="Argument",
        description=(
            "Argument to be passed to the rollback tool."
            "It can be any string like the last command it rolled back, or at least provide a random string."
        ),
    )
...
class RollbackTool(BaseTool):
    ...
    name: str = "rollback_tool"
    description: str = "Rolls back the last agent action using the action stack."
    action_stack: Optional[object] = Field(default=None, exclude=True)
    config: StratusAgentConfig = Field(default=None, exclude=True)
    args_schema: Type[BaseModel] = RollbackToolSchema
    cache_function: Callable = lambda _args=None, _result=None: False

    def __init__(self, action_stack, config: StratusAgentConfig):
        super().__init__(
            name="rollback_tool",
            description="Rolls back the last agent action using the action stack.",
        )
        self.action_stack = action_stack
        self.config = config

    @field_serializer("action_stack")
    def serialize_action_stack(self, action_stack):
        """Skip serialization for action_stack."""
        return None
        ...
        try:
            ...
            if last_action is not None:
                ...
                if self.config.validate_rollback:
                    ...
                    current_state = _cleanup_kubernetes_yaml(current_state)
