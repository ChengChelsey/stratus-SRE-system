...
class LLMAnalyzerPrioritized(BaseTool):
    name: str = "Summarize content and key insights."
    description: str = "A tool that be used to summarize the content given and list key insights."
    llm_backend: Any
    ...
    def _run(self, content_to_summarize: str) -> str:
        input = content_to_summarize
        system_prompt = "You are tasked with analyzing the content provided, these are the thought history of the last run of the agent. Please extract the root cause and the mitigation plan only. Please pay more attention to the latter part of the content because it contains the most important information. Because they are the most recent, the most relevant, and the most essential. Do not summarize the content. The root cause and the mitigation plan is the only thing you need to extract."
        try:
            response = self.llm_backend.inference(system_prompt, input)
            return response
        except Exception as e:
            print(f"LLMAnalyzerPrioritized error summarizing: {str(e)}")
            logger.error(f"LLMAnalyzerPrioritized error summarizing: {str(e)}")
            return None
