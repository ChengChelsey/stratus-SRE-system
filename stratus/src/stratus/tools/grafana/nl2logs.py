...
from stratus.tools.linting.logql_linter import LogQLLinter
...
class NL2LogsCustomTool(BaseTool, GrafanaBaseClient):
    ...
    def _run(self, nl_query: str) -> str:
        ...
        try:
            function_name, function_arguments = self._generate_logql_query(prompt=nl_query)
    ...
    def _get_labels(self):
        try:
            datasource_id = self.get_datasource_id("loki")
            url = f"{self.grafana_url}/api/datasources/proxy/uid/{datasource_id}/api/v1/query_range"
    ...
    def _get_label_value_dict(self):
        try:
            ...
            for label in response.json()["data"]:
                value_url = (
                    f"{self.grafana_url}/api/datasources/proxy/uid/{datasource_id}/loki/api/v1/label/{label}/values"
                )
