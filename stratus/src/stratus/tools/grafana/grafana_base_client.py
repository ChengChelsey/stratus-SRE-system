...
class GrafanaBaseClient:
    ...
    def model_post_init(self):
        self.grafana_url = os.environ.get(
            "OBSERVABILITY_STACK_URL", "http://localhost:8080/grafana/api/prometheus/grafana"
        )
        self.topology_url = os.environ.get("TOPOLOGY_URL", "http://localhost:8081")
        # service account token is created on grafana dashboard _manually_
        self.grafana_service_account_token = os.environ.get("GRAFANA_SERVICE_ACCOUNT_TOKEN", "NOP")

        logger.debug("url: {g}, token: {t}".format(g=self.grafana_url, t=self.grafana_service_account_token))
