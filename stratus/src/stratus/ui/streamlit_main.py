...
def callback_crewai_task(output: TaskOutput):
    message = {"role": "Stratus-SRE-Agent", "content": str(output.raw)}
    st.session_state.messages.append(message)
    st.chat_message("Stratus-SRE-Agent").write(message)


def callback_crewai_agent(output: AgentAction):
    to_show = f"{output.thought}\n Tool: {output.tool}\n Tool Input: {output.tool_input}\n Result: {output.result}"
    message = {"role": "Stratus-SRE-Agent", "content": to_show}
    st.session_state.messages.append(message)
    st.chat_message("Stratus-SRE-Agent").write(message)
...
def initialize_crew(prompt):
    try:
        ...
        result = StratusCrew(callback_agent=callback_agent, callback_task=callback_task).crew().kickoff(inputs=inputs)

        message = {"role": "Stratus-SRE-Agent", "content": "## Final Result\n" + str(result.raw)}
        st.session_state.messages.append(message)
        st.chat_message("Stratus-SRE-Agent").write(message)
...
if __name__ == "__main__":
    st.title("Stratus - Your Site Reliability Engineering (SRE) Agent")
    # Initializing the message array (if not already present)
    if "messages" not in st.session_state:
        st.session_state["messages"] = [
            {"role": "assistant", "content": "What alert(s) are you observering on the IT environment?"}
        ]
