...
def callback(contents: str, user: str, instance: pn.chat.ChatInterface):
    global initiate_chat_task_created  # noqa F824
    global user_input
...
def callback_crewai_task(output: TaskOutput):
    chat_interface.send(str(output.raw), user="Stratus-SRE-Agent", respond=False)


def callback_crewai_agent(output: AgentAction):
    to_show = f"{output.thought}\n Tool: {output.tool}\n Tool Input: {output.tool_input}\n Result: {output.result}"
    chat_interface.send(to_show, user="Stratus-SRE-Agent", respond=False)
